...
TEST(ParseTest, SmallInstance) {
  ...
  const std::vector<std::vector<double>> expect_supply_costs = {
      {0, 1, 2, 3}, {40, 50, 60, 70}, {1.1, 11.1, 111.0, 1111.0}};
  ...
  for (int i = 0; i < problem.num_customers; i++) {
    EXPECT_THAT(problem.supply_costs[i],
                ElementsAreArray(expect_supply_costs[i]));
  }
}
...
TEST(UFLSolverTest, TwoFacilities) {
  ...
  problem.supply_costs = {{1.0, 0.5}, {0.5, 1.0}};
  ...
  EXPECT_THAT(solution.open_values,
              Pointwise(DoubleNear(kTolerance), expect_open));
...
TEST(UFLSolverTest, OnlySupply) {
  ...
  problem.supply_costs = {{1, 2}, {2, 1}, {2, 3}, {3, 4}};
  UFLBenders solver(problem, math_opt::SolverType::kGscip);
